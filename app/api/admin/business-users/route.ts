...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // Business user ekle (membership bilgisiyle birlikte)
      const userResult = await query(
        `INSERT INTO business_users (
          email, password_hash, full_name, phone, role, added_by_admin, is_active, email_verified,
          membership_type, max_cameras, membership_expiry_date, license_key
        ) VALUES ($1, $2, $3, $4, 'business_user', true, true, true, $5, $6, $7, $8)
        RETURNING id`,
        [
          email, 
          passwordHash, 
          fullName, 
          phone || null,
          planType,           // membership_type (premium/enterprise)
          config.maxCameras,  // max_cameras (10/50)
      ...
      console.log(`📸 User ${userId} - ${planType} plan: ${config.maxCameras} kamera limiti atandı`);
