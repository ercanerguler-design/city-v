...
export default function TensorFlowAIAnalysis({
  ...
  useEffect(() => {
    ...
    if (!video) {
      console.log('⚠️ TensorFlow: Video ref not ready');
      return;
    }

    console.log('🎥 TensorFlow: Setting up MJPEG stream from:', streamUrl);
    ...
    img.onload = () => {
      console.log('✅ TensorFlow: MJPEG stream loaded', {
    ...
    img.onerror = (e) => {
      console.error('❌ TensorFlow: MJPEG stream failed to load', e);
    };
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      // Check if video has dimensions (either videoWidth or canvas drawn content)
      if (!video.videoWidth && !video.width) {
        // Log warning only once
        if (frameCountRef.current === 0) {
          console.log('⚠️ TensorFlow: Waiting for video dimensions...');
        }
      ...
      try {
        ...
        if (frameCountRef.current === 1) {
          console.log('🎯 TensorFlow: First detection completed', {
